...
${_listarPalavrasUnicas(palavras)}
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) =>
            TelaResultados(textoOriginal: texto, resultado: resultadoFormatado),
      ),
    );
  }
  ...
  String _listarPalavrasUnicas(List<String> palavras) {
    ...
    for (var palavra in palavras) {
      final palavraLimpa = palavra.trim().toLowerCase().replaceAll(
        RegExp(r'[^\w]'),
        '',
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          'Bem-vindo, ${widget.usuario.nomeCompleto.split(' ').first}!',
        ),
        actions: [
          IconButton(
